...
def graph(I, O, C, PP):
    ...
    G = empty(n, n)
    for i in I:
        if max(PP[i-1]) and favo(i, O, PP):
            G[i-1][belong(favo(i, O, PP), I, C)-1] = 1
    return G
...
def HEC(CC, PP):
    ...
    while len(all_out)<len(O[0]):

        t += 1
        
        # 得到本轮TTC圈中人和物
        out = TTC(I[t-1], O[t-1], allocate(I[t-1], O[t-1], C[t-1]), PP)
        all_out += out[0]
        #print('第%s期出圈人\n'%t)
        #output(out)
